...
Void FitLine2DByLeastSquares(std::map<int, std::vector<double>> mapPoint, double &k, double &b)
{
	...
	k = (pCount*sumXY - sumX * sumY) / (pCount*sumXX - sumX * sumX);
	b = (sumY*sumXX - sumX * sumXY) / (pCount*sumXX - sumX * sumX);
}
...
int main(int argc, char **argv)
{
  ...
  for (auto i = 0; i < cloud->size(); ++i)
  {
    ...
    else
    {
      ...
      std::cout << "x y z " << (*cloud)[i].x  << (*cloud)[i].y  << (*cloud)[i].z << std::endl;
    }
  }
  ...
  if (pcl::console::find_argument(argc, argv, "-f") >= 0)
  {
    ...
    std::cout << "coefficients " << ransac.model_coefficients_ << std::endl;
  }
